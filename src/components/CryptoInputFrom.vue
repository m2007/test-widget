<template>
  <div class="crcontainer" :class="{'crcontainer--active rounded-b-none': search}">
      <div class="crform">
        <input class="crform_input" :class="{'crform_input--close': search}"
        :type="(search ? 'text' : 'number')" :placeholder="(search ? 'Search' : '')" @blur="onBlurFrom"
        v-model="value">
...
export default {
  ...
  watch: {
    minAmount() {
      this.amountStr = this.minAmount
      if (!this.search) {
        this.value = this.amountStr
      }
    },
    value() {
  ...
  async mounted() {
    this.activeCrypto = this.cryptos[0]
    this.value = this.amountStr = this.minAmount
  },
  methods: {
    onBlurFrom() {
      if (!this.search) {
        this.$emit('updatedNewAmountFrom', this.value.toString())
      }
    },
    toogleInput() {
